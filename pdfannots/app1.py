...
class App:
    def __init__(self):
        ...
        self.open_file_button = Button(self.root, text='Open PDF File', command=self.event_open_file)
        self.open_file_button.grid(row=0, column=0, rowspan=1, columnspan=1)

        self.open_preview_button = Button(self.root, text='Open Preview', command=self.event_open_preview)
        self.open_preview_button.grid(row=0, column=1, rowspan=1, columnspan=1)

        self.export_annotations_button = Button(self.root, text='Export', command=self.event_export)
        self.export_annotations_button.grid(row=0, column=2, rowspan=1, columnspan=1)

        self.file_name_label = Label(self.root, text='...', borderwidth=1, relief="ridge")

        # Init Frame for Annotation Preview
        self.annotation_preview_frame = Frame(self.root)
